...
contract Zigazougou is ERC721, ERC721Enumerable, ERC721URIStorage {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    uint256 MAX_SUPPLY = 10000;

    // Let the users mint only up to a certain number of NFTs
    // We need to store the amount of tokens every address minted
    mapping(address => uint256) nftsToAddress;

    constructor() ERC721("Zigazougou", "ZGOU") {}

    function safeMint(address to, string memory uri) public {
        // Check if the number of NFT by address is less than the maximum authorized
        require(nftsToAddress[to] < 5, "You reached the total amount of NFTs that you can mint!");
